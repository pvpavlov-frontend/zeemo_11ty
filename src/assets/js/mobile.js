...
(function() {
  if(document.addEventListener && window.isMediaQueriesSupported) {
    ...
    var animation = window.supportsCSS('transition-property', 'all');
    
    if(animation) {
      burgerButtonContainer.innerHTML = '<button class="burger__button burger__button--animated" id="burger-button" aria-haspopup="true" aria-expanded="false" aria-label="Open menu"><span class="burger__button-row"></span></button>';
    } else {
